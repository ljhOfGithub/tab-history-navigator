...
**Tab History Navigator** 高度重视您的隐私：

✅ **本地存储**：所有标签页历史记录（URL、标题、时间戳）仅存储在您的设备本地，使用 Chrome 的 storage.local API。

❌ **不上传**：我们**不会**收集、上传或分享任何用户数据到任何服务器。

❌ **无追踪**：本扩展不含任何分析工具、追踪代码或第三方 SDK。

🔄 **透明可控**：您可以随时通过弹出窗口的"清除历史"按钮删除所有记录。

📅 卸载扩展后，所有相关数据将自动从您的设备中删除。
